...
2. GitHub連携 → Claude GitHub App を対象リポジトリにインストール
3. Environment を作成し、以下を設定
   - **Network access**: Plane/Discordに到達できるように（Full もしくは Planeホスト + Discord を許可）
   - **Environment variables**: 例
     - `PLANE_API_HOST_URL=...`
     - `PLANE_API_KEY=...`
     - `PLANE_WORKSPACE_SLUG=...`
     - `PLANE_PROJECT_ID=...`
     - `DISCORD_WEBHOOK_URL=...`
4. Claude Code Webのタスクで、以下を実行（SSOTは人間が用意）

```bash
# multi-repo前提のため、common/saasを用意（場所は任意）
git clone <hotel-common-rebuild のURL> /work/repos/hotel-common-rebuild
git clone <hotel-saas-rebuild のURL> /work/repos/hotel-saas-rebuild

# v3にパスを教える（kanriはこのリポジトリ）
export AUTODEV_COMMON_DIR="/work/repos/hotel-common-rebuild"
export AUTODEV_SAAS_DIR="/work/repos/hotel-saas-rebuild"
```
...
ポイント:
- この運用では **SSOT系サブタスクはスキップ**されます（デフォルト）。SSOTは人間が作成し、Planeに `ssot:` を設定しておく運用が安定です。
- PR作成対象はデフォルトで `common,saas` のみ（kanriの差分混入を防止）。

### PCを閉じても回す（サーバー側で回す）

**重要**: PCを閉じても動かすには、常時稼働する実行環境が必要です（Claude.ai Background / VPS / 常時稼働Mac 等）。
...
- [Plane API](../plane/README_PLANE_API.md)
