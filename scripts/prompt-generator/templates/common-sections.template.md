...
```bash
echo "=== {{TASK_ID}} 実行ログ ===" > evidence/{{TASK_ID}}/commands.log

# 実行したコマンドを記録
echo "$ npm run dev" >> evidence/{{TASK_ID}}/commands.log
echo "Exit code: $?" >> evidence/{{TASK_ID}}/commands.log
```

### Evidence 2: Files
```bash
echo "=== 作成/変更ファイル ===" > evidence/{{TASK_ID}}/files.log
git status --short >> evidence/{{TASK_ID}}/files.log
ls -la <作成ファイル> >> evidence/{{TASK_ID}}/files.log
```

### Evidence 3: Git
```bash
echo "=== Git状態 ===" > evidence/{{TASK_ID}}/git.log
git branch --show-current >> evidence/{{TASK_ID}}/git.log
git log --oneline -3 >> evidence/{{TASK_ID}}/git.log
```

### Evidence 4: Test
```bash
echo "=== テスト結果 ===" > evidence/{{TASK_ID}}/test.log
./scripts/test-standard-guest.sh >> evidence/{{TASK_ID}}/test.log 2>&1
# または
./scripts/test-standard-admin.sh >> evidence/{{TASK_ID}}/test.log 2>&1
```
...
### Evidence
- `evidence/{{TASK_ID}}/commands.log`
- `evidence/{{TASK_ID}}/files.log`
- `evidence/{{TASK_ID}}/git.log`
- `evidence/{{TASK_ID}}/test.log`
