# DEV-0170-TEST: AIチャットでは対応困難な問い合わせを、60秒以内にスタッフ

**タスクタイプ**: fullstack
**推定工数**: 43時間
**生成日時**: 2026-01-20T04:48:49.246Z
...
以下のエージェント役割を意識して実装してください：

### code-reviewer
コードレビューの専門家。品質、保守性、パフォーマンスを評価します。
# Code Reviewer Agent

### planner
実装計画を立案する専門家。タスクを分解し、依存関係を整理します。
# Planner Agent

### security-reviewer
セキュリティ監査の専門家。脆弱性を検出し、修正を提案します。
# Security Reviewer Agent

### tdd-guide
テスト駆動開発（TDD）の専門家。テストを先に書き、実装を導きます。
# TDD Guide Agent
...
```bash
echo "=== DEV-0170-TEST 実行ログ ===" > evidence/DEV-0170-TEST/commands.log

# 実行したコマンドを記録
echo "$ npm run dev" >> evidence/DEV-0170-TEST/commands.log
echo "Exit code: $?" >> evidence/DEV-0170-TEST/commands.log
```

### Evidence 2: Files
```bash
echo "=== 作成/変更ファイル ===" > evidence/DEV-0170-TEST/files.log
git status --short >> evidence/DEV-0170-TEST/files.log
ls -la <作成ファイル> >> evidence/DEV-0170-TEST/files.log
```

### Evidence 3: Git
```bash
echo "=== Git状態 ===" > evidence/DEV-0170-TEST/git.log
git branch --show-current >> evidence/DEV-0170-TEST/git.log
git log --oneline -3 >> evidence/DEV-0170-TEST/git.log
```

### Evidence 4: Test
```bash
echo "=== テスト結果 ===" > evidence/DEV-0170-TEST/test.log
./scripts/test-standard-guest.sh >> evidence/DEV-0170-TEST/test.log 2>&1
# または
./scripts/test-standard-admin.sh >> evidence/DEV-0170-TEST/test.log 2>&1
```
...
### Evidence
- `evidence/DEV-0170-TEST/commands.log`
- `evidence/DEV-0170-TEST/files.log`
- `evidence/DEV-0170-TEST/git.log`
- `evidence/DEV-0170-TEST/test.log`
...
# DEV-0170-TEST: AIチャットでは対応困難な問い合わせを、60秒以内にスタッフ - Backend API実装
...
| ID | 名前 | タイプ | Accept条件 |
|:---|:-----|:-------|:-----------|
| HDF-001 | ハンドオフリクエスト作成 | FR | tenantId、sessionId、roomId、channel、contextを含むリクエストが |
...
| HDF-202 | アクセシビリティ | UI |  |
| HDF-300 | ROI | BIZ |  |
| HDF-301 | KPI | BIZ | HandoffRequestモデル作成（hotel-common）, ゲスト向けAPI実装（POST |
...
| Method | Path | 説明 |
|:-------|:-----|:-----|
| POST | `/api/v1/guest/handoff/requests` | ハンドオフリクエストを作成 |
| GET | `/api/v1/guest/handoff/requests/:id` | 自分のリクエスト詳細を取得 |
| GET | `/api/v1/admin/handoff/requests` | リクエスト一覧を取得 |
| GET | `/api/v1/admin/handoff/requests/:id` | リクエスト詳細を取得 |
| PATCH | `/api/v1/admin/handoff/requests/:id/status` | ステータスを更新 |
 ...
 * ハンドオフリクエストを作成
 */
router.post('/handoff/requests', async (req: Request, res: Response) => {
  try {
    ...
    // TODO: ビジネスロジック実装
    const result = await prisma.handoffRequest.create({
      where: { tenant_id: tenantId }
    });

    return res.status(201).json(createSuccessResponse(result));
  } catch (error) {
    console.error('/api/v1/guest/handoff/requests エラー:', error);
 ...
 * 自分のリクエスト詳細を取得
 */
router.get('/handoff/requests/:id', async (req: Request, res: Response) => {
  try {
    const tenantId = req.headers['x-tenant-id'] as string;
    if (!tenantId) {
      return res.status(401).json(createErrorResponse('UNAUTHORIZED', 'テナントIDが必要です'));
    ...
    // TODO: ビジネスロジック実装
    const result = await prisma.handoffRequest.findMany({
      where: { tenant_id: tenantId }
    });

    return res.status(200).json(createSuccessResponse(result));
  } catch (error) {
    console.error('/api/v1/guest/handoff/requests/:id エラー:', error);
    return res.status(500).json(createErrorResponse('INTERNAL_ERROR', 'サーバーエラー'));
  }
});
 ...
 * リクエスト一覧を取得
 */
router.get('/api/v1/admin/handoff/requests', async (req: Request, res: Response) => {
  try {
    const tenantId = req.headers['x-tenant-id'] as string;
    if (!tenantId) {
      return res.status(401).json(createErrorResponse('UNAUTHORIZED', 'テナントIDが必要です'));
    ...
    console.error('/api/v1/admin/handoff/requests エラー:', error);
    return res.status(500).json(createErrorResponse('INTERNAL_ERROR', 'サーバーエラー'));
  }
});
...
/**
 * GET /api/v1/admin/handoff/requests/:id
 * リクエスト詳細を取得
 */
router.get('/api/v1/admin/handoff/requests/:id', async (req: Request, res: Response) => {
  try {
    const tenantId = req.headers['x-tenant-id'] as string;
    if (!tenantId) {
      return res.status(401).json(createErrorResponse('UNAUTHORIZED', 'テナントIDが必要です'));
    ...
    console.error('/api/v1/admin/handoff/requests/:id エラー:', error);
    return res.status(500).json(createErrorResponse('INTERNAL_ERROR', 'サーバーエラー'));
  }
});
 ...
 * ステータスを更新
 */
router.patch('/api/v1/admin/handoff/requests/:id/status', async (req: Request, res: Response) => {
  try {
    const tenantId = req.headers['x-tenant-id'] as string;
    if (!tenantId) {
      return res.status(401).json(createErrorResponse('UNAUTHORIZED', 'テナントIDが必要です'));
    ...
    // TODO: ビジネスロジック実装
    const result = await prisma.handoffRequest.update({
      where: { tenant_id: tenantId }
    });

    return res.status(200).json(createSuccessResponse(result));
  } catch (error) {
    console.error('/api/v1/admin/handoff/requests/:id/status エラー:', error);
    return res.status(500).json(createErrorResponse('INTERNAL_ERROR', 'サーバーエラー'));
  }
});
...
app.use('/api/v1/guest', handoffRouter);
```
...
### Step 1: プロキシファイル作成
```bash
mkdir -p hotel-saas-rebuild/server/api/v1/guest
```
...
export default defineEventHandler(async (event) => {
  ...
  const response = await callHotelCommonAPI(event, `/api/v1/guest/handoff/requests`, {
    method: 'POST',
...
export default defineEventHandler(async (event) => {
  ...
  const response = await callHotelCommonAPI(event, `/api/v1/guest/handoff/requests/${id}`, {
    method: 'GET',
    headers: { 'x-tenant-id': tenantId }
  });
...
ファイル: `server/api/v1/guest/api/v1/admin/handoff/requests.get.ts`
...
export default defineEventHandler(async (event) => {
  ...
  const response = await callHotelCommonAPI(event, `/api/v1/admin/handoff/requests`, {
    method: 'GET',
    headers: { 'x-tenant-id': tenantId }
  });
...
ファイル: `server/api/v1/guest/api/v1/admin/handoff/requests/[id].get.ts`
...
export default defineEventHandler(async (event) => {
  ...
  const response = await callHotelCommonAPI(event, `/api/v1/admin/handoff/requests/${id}`, {
    method: 'GET',
    headers: { 'x-tenant-id': tenantId }
  });
...
ファイル: `server/api/v1/guest/api/v1/admin/handoff/requests/[id]/status.patch.ts`
...
export default defineEventHandler(async (event) => {
  ...
  const response = await callHotelCommonAPI(event, `/api/v1/admin/handoff/requests/${id}/status`, {
    method: 'PATCH',
    headers: { 'x-tenant-id': tenantId },
    body: await readBody(event)
  });
...
```bash
# POST /api/v1/guest/handoff/requests
curl -s -X POST http://localhost:3401/api/v1/guest/handoff/requests \
...
```bash
# GET /api/v1/guest/handoff/requests/:id
curl -s http://localhost:3401/api/v1/guest/handoff/requests/1 \
  -H 'x-tenant-id: tenant-003bc06e-4ea0-4f93-9ce2-bf56dfe237b7' | jq .
```

```bash
# GET /api/v1/admin/handoff/requests
curl -s http://localhost:3401/api/v1/admin/handoff/requests \
  -H 'x-tenant-id: tenant-003bc06e-4ea0-4f93-9ce2-bf56dfe237b7' | jq .
```

```bash
# GET /api/v1/admin/handoff/requests/:id
curl -s http://localhost:3401/api/v1/admin/handoff/requests/1 \
  -H 'x-tenant-id: tenant-003bc06e-4ea0-4f93-9ce2-bf56dfe237b7' | jq .
```

```bash
# PATCH /api/v1/admin/handoff/requests/:id/status
curl -s -X PATCH http://localhost:3401/api/v1/admin/handoff/requests/1/status \
  -H 'x-tenant-id: tenant-003bc06e-4ea0-4f93-9ce2-bf56dfe237b7' \
  -H 'Content-Type: application/json' \
  -d '{"key": "value"}' | jq .
```
...
```bash
# ゲストAPI用
./scripts/test-standard-guest.sh 2>&1 | tee evidence/DEV-0170-TEST/test.log

# または管理画面API用
./scripts/test-standard-admin.sh 2>&1 | tee evidence/DEV-0170-TEST/test.log
```

### Step 2: Evidence保存
```bash
mkdir -p evidence/DEV-0170-TEST
echo "=== DEV-0170-TEST Evidence ===" > evidence/DEV-0170-TEST/commands.log
git status --short >> evidence/DEV-0170-TEST/commands.log
ls -la hotel-common-rebuild/src/routes/handoff.routes.ts >> evidence/DEV-0170-TEST/commands.log
```
...
- `hotel-saas-rebuild/server/api/v1/guest/`

### テスト結果
- 標準テスト: ✅ PASS

### Evidence
- `evidence/DEV-0170-TEST/`
```
...
- [ ] ❌ hotel-saasでPrisma使用禁止
- [ ] ⚠️ console.logが残っています
- [ ] ❌ フォールバック値禁止
- [ ] ⚠️ any型の使用を避けてください
- [ ] ❌ $fetch直接使用禁止。callHotelCommonAPIを使用してください
