# DEV-0201: AIチャットでは対応困難な問い合わせを、60秒以内にスタッフ

**タスクタイプ**: fullstack
**推定工数**: 27時間
**生成日時**: 2026-01-18T00:16:41.380Z
...
# DEV-0201: AIチャットでは対応困難な問い合わせを、60秒以内にスタッフ - Backend API実装
...
```bash
# ログ保存
mkdir -p evidence/DEV-0201
curl ... > evidence/DEV-0201/api-test.log 2>&1
```
...
```bash
# ゲストAPI用
./scripts/test-standard-guest.sh 2>&1 | tee evidence/DEV-0201/test-standard.log

# 管理画面API用
./scripts/test-standard-admin.sh 2>&1 | tee evidence/DEV-0201/test-standard.log
```

### Step 2: 結果確認
```bash
# PASSを確認
grep -E "✅|❌|PASS|FAIL" evidence/DEV-0201/test-standard.log
```
...
```bash
echo "=== 実行コマンド ===" > evidence/DEV-0201/commands.log
# 実行したコマンドと結果を記録
```
