# BATCH-STAFF: SAAS STAFF MANAGEMENT

**タスクタイプ**: fullstack
**推定工数**: 47時間
**生成日時**: 2026-01-19T22:27:40.198Z
...
```bash
echo "=== BATCH-STAFF 実行ログ ===" > evidence/BATCH-STAFF/commands.log

# 実行したコマンドを記録
echo "$ npm run dev" >> evidence/BATCH-STAFF/commands.log
echo "Exit code: $?" >> evidence/BATCH-STAFF/commands.log
```

### Evidence 2: Files
```bash
echo "=== 作成/変更ファイル ===" > evidence/BATCH-STAFF/files.log
git status --short >> evidence/BATCH-STAFF/files.log
ls -la <作成ファイル> >> evidence/BATCH-STAFF/files.log
```

### Evidence 3: Git
```bash
echo "=== Git状態 ===" > evidence/BATCH-STAFF/git.log
git branch --show-current >> evidence/BATCH-STAFF/git.log
git log --oneline -3 >> evidence/BATCH-STAFF/git.log
```

### Evidence 4: Test
```bash
echo "=== テスト結果 ===" > evidence/BATCH-STAFF/test.log
./scripts/test-standard-guest.sh >> evidence/BATCH-STAFF/test.log 2>&1
# または
./scripts/test-standard-admin.sh >> evidence/BATCH-STAFF/test.log 2>&1
```
...
### Evidence
- `evidence/BATCH-STAFF/commands.log`
- `evidence/BATCH-STAFF/files.log`
- `evidence/BATCH-STAFF/git.log`
- `evidence/BATCH-STAFF/test.log`
...
# BATCH-STAFF: SAAS STAFF MANAGEMENT - Backend API実装
...
| ドキュメント | パス |
|:------------|:-----|
| **メインSSOT** | `docs/03_ssot/00_foundation/SSOT_SAAS_STAFF_MANAGEMENT.md` |
...
| Method | Path | 説明 |
|:-------|:-----|:-----|
| GET | `/api/v1/admin/staff`` | コードブロックから抽出 |
| GET | `/api/v1/admin/staff/:id`` | コードブロックから抽出 |
| POST | `/api/v1/admin/staff/:id/tenants`` | コードブロックから抽出 |
...
```bash
# SSOT読み込み
cat docs/03_ssot/00_foundation/SSOT_SAAS_STAFF_MANAGEMENT.md

# API定義を確認
grep -nE '^(GET|POST|PUT|PATCH|DELETE)' docs/03_ssot/00_foundation/SSOT_SAAS_STAFF_MANAGEMENT.md
```
...
### Step 1: ファイル作成
```bash
touch hotel-common-rebuild/src/routes/management.routes.ts
```
...
router.get('/staff`', async (req: Request, res: Response) => {
  try {
    ...
    console.error('/api/v1/admin/staff` エラー:', error);
...
router.get('/staff/:id`', async (req: Request, res: Response) => {
  try {
    ...
    console.error('/api/v1/admin/staff/:id` エラー:', error);
    return res.status(500).json(createErrorResponse('INTERNAL_ERROR', 'サーバーエラー'));
  }
});
 ...
 * コードブロックから抽出
 */
router.post('/staff/:id/tenants`', async (req: Request, res: Response) => {
  try {
    const tenantId = req.headers['x-tenant-id'] as string;
    if (!tenantId) {
      return res.status(401).json(createErrorResponse('UNAUTHORIZED', 'テナントIDが必要です'));
    ...
    console.error('/api/v1/admin/staff/:id/tenants` エラー:', error);
    return res.status(500).json(createErrorResponse('INTERNAL_ERROR', 'サーバーエラー'));
  }
});
...
app.use('/api/v1/admin', managementRouter);
```

### Step 4: 完了条件
- [ ] `management.routes.ts` が作成された
- [ ] `index.ts` にルーター登録された
- [ ] TypeScript型エラーがない
...
### Step 1: プロキシファイル作成
```bash
mkdir -p hotel-saas-rebuild/server/api/v1/admin
```
...
ファイル: `server/api/v1/admin/staff`.get.ts`
...
export default defineEventHandler(async (event) => {
  ...
  const response = await callHotelCommonAPI(event, `/api/v1/admin/staff``, {
    method: 'GET',
...
ファイル: `server/api/v1/admin/staff/[id]`.get.ts`
...
export default defineEventHandler(async (event) => {
  ...
  const response = await callHotelCommonAPI(event, `/api/v1/admin/staff/${id}``, {
    method: 'GET',
    headers: { 'x-tenant-id': tenantId }
  });
...
ファイル: `server/api/v1/admin/staff/[id]/tenants`.post.ts`
...
export default defineEventHandler(async (event) => {
  ...
  const response = await callHotelCommonAPI(event, `/api/v1/admin/staff/${id}/tenants``, {
    method: 'POST',
    headers: { 'x-tenant-id': tenantId },
...
```bash
# GET /api/v1/admin/staff`
curl -s http://localhost:3401/api/v1/admin/staff` \
  -H 'x-tenant-id: tenant-003bc06e-4ea0-4f93-9ce2-bf56dfe237b7' | jq .
```

```bash
# GET /api/v1/admin/staff/:id`
curl -s http://localhost:3401/api/v1/admin/staff/1` \
  -H 'x-tenant-id: tenant-003bc06e-4ea0-4f93-9ce2-bf56dfe237b7' | jq .
```

```bash
# POST /api/v1/admin/staff/:id/tenants`
curl -s -X POST http://localhost:3401/api/v1/admin/staff/1/tenants` \
...
```bash
# ゲストAPI用
./scripts/test-standard-guest.sh 2>&1 | tee evidence/BATCH-STAFF/test.log

# または管理画面API用
./scripts/test-standard-admin.sh 2>&1 | tee evidence/BATCH-STAFF/test.log
```

### Step 2: Evidence保存
```bash
mkdir -p evidence/BATCH-STAFF
echo "=== BATCH-STAFF Evidence ===" > evidence/BATCH-STAFF/commands.log
git status --short >> evidence/BATCH-STAFF/commands.log
ls -la hotel-common-rebuild/src/routes/management.routes.ts >> evidence/BATCH-STAFF/commands.log
```
...
### 実装成果物
- `hotel-common-rebuild/src/routes/management.routes.ts`
- `hotel-saas-rebuild/server/api/v1/admin/`

### テスト結果
- 標準テスト: ✅ PASS

### Evidence
- `evidence/BATCH-STAFF/`
```
