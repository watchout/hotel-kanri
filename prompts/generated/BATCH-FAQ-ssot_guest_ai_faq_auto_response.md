# BATCH-FAQ: GUEST AI FAQ AUTO RESPONSE

**タスクタイプ**: fullstack
**推定工数**: 10時間
**生成日時**: 2026-01-19T22:24:35.182Z
...
```bash
echo "=== BATCH-FAQ 実行ログ ===" > evidence/BATCH-FAQ/commands.log

# 実行したコマンドを記録
echo "$ npm run dev" >> evidence/BATCH-FAQ/commands.log
echo "Exit code: $?" >> evidence/BATCH-FAQ/commands.log
```

### Evidence 2: Files
```bash
echo "=== 作成/変更ファイル ===" > evidence/BATCH-FAQ/files.log
git status --short >> evidence/BATCH-FAQ/files.log
ls -la <作成ファイル> >> evidence/BATCH-FAQ/files.log
```

### Evidence 3: Git
```bash
echo "=== Git状態 ===" > evidence/BATCH-FAQ/git.log
git branch --show-current >> evidence/BATCH-FAQ/git.log
git log --oneline -3 >> evidence/BATCH-FAQ/git.log
```

### Evidence 4: Test
```bash
echo "=== テスト結果 ===" > evidence/BATCH-FAQ/test.log
./scripts/test-standard-guest.sh >> evidence/BATCH-FAQ/test.log 2>&1
# または
./scripts/test-standard-admin.sh >> evidence/BATCH-FAQ/test.log 2>&1
```
...
### Evidence
- `evidence/BATCH-FAQ/commands.log`
- `evidence/BATCH-FAQ/files.log`
- `evidence/BATCH-FAQ/git.log`
- `evidence/BATCH-FAQ/test.log`
...
# BATCH-FAQ: GUEST AI FAQ AUTO RESPONSE - Backend API実装
...
| ドキュメント | パス |
|:------------|:-----|
| **メインSSOT** | `docs/03_ssot/02_guest_features/ai_chat/SSOT_GUEST_AI_FAQ_AUTO_RESPONSE.md` |
...
| Method | Path | 説明 |
|:-------|:-----|:-----|
| POST | `/api/v1/ai/feedback` | コードブロックから抽出 |
| POST | `/api/v1/ai/feedback`` | コードブロックから抽出 |
...
```bash
# SSOT読み込み
cat docs/03_ssot/02_guest_features/ai_chat/SSOT_GUEST_AI_FAQ_AUTO_RESPONSE.md

# API定義を確認
grep -nE '^(GET|POST|PUT|PATCH|DELETE)' docs/03_ssot/02_guest_features/ai_chat/SSOT_GUEST_AI_FAQ_AUTO_RESPONSE.md
```
...
### Step 1: ファイル作成
```bash
touch hotel-common-rebuild/src/routes/response.routes.ts
```
...
router.post('/feedback', async (req: Request, res: Response) => {
  try {
    ...
    // TODO: ビジネスロジック実装
    const result = await prisma.faqResponse.create({
      where: { tenant_id: tenantId }
    });

    return res.status(201).json(createSuccessResponse(result));
  } catch (error) {
    console.error('/api/v1/ai/feedback エラー:', error);
...
app.use('/api/v1/ai', responseRouter);
```

### Step 4: 完了条件
- [ ] `response.routes.ts` が作成された
- [ ] `index.ts` にルーター登録された
- [ ] TypeScript型エラーがない
...
### Step 1: プロキシファイル作成
```bash
mkdir -p hotel-saas-rebuild/server/api/v1/ai
```
...
ファイル: `server/api/v1/ai/feedback.post.ts`
...
export default defineEventHandler(async (event) => {
  ...
  const response = await callHotelCommonAPI(event, `/api/v1/ai/feedback`, {
    method: 'POST',
...
```bash
# POST /api/v1/ai/feedback
curl -s -X POST http://localhost:3401/api/v1/ai/feedback \
...
```bash
# ゲストAPI用
./scripts/test-standard-guest.sh 2>&1 | tee evidence/BATCH-FAQ/test.log

# または管理画面API用
./scripts/test-standard-admin.sh 2>&1 | tee evidence/BATCH-FAQ/test.log
```

### Step 2: Evidence保存
```bash
mkdir -p evidence/BATCH-FAQ
echo "=== BATCH-FAQ Evidence ===" > evidence/BATCH-FAQ/commands.log
git status --short >> evidence/BATCH-FAQ/commands.log
ls -la hotel-common-rebuild/src/routes/response.routes.ts >> evidence/BATCH-FAQ/commands.log
```
...
### 実装成果物
- `hotel-common-rebuild/src/routes/response.routes.ts`
- `hotel-saas-rebuild/server/api/v1/ai/`
