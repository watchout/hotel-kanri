...
|-----------|-----|----------|
| §1 要件ID一覧 | [CORE] | - |
| §2-§5 概要・スコープ・基本方針・用語 | [CORE] | - |
| §6 要件定義 | [CONTRACT] | MUST |
| §7 データベース設計 | [CONTRACT] | MUST |
| §8 API仕様 | [CONTRACT] | MUST |
| §9 フロントエンド実装 | [DETAIL] | SHOULD |
| §10 セキュリティ詳細 | [CORE] | MUST |
| §11-§13 テスト・マイグレーション・チェックリスト | [DETAIL] | MAY |
