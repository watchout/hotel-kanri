# 📋 Plane Issues定義（リビルドプロジェクト）

**最終更新**: 2025年11月4日  
**目的**: リビルドプロジェクトのPlane Issue完全定義  
...
```
Plane Issue作成の完全マニュアル

✅ やること:
1. Plane Projectを作成
2. このドキュメントの各IssueをPlaneに登録
3. 依存関係（Blocked by）を設定
4. Phaseごとに実装開始

❌ やらないこと:
- このファイルで進捗管理（Planeのみ）
- 手動でIssue内容を変更（Planeで更新）
```
...
- [ ] Phase 6: CRUD Verify（結果ログ・スクショをPlaneに添付）
- [ ] CI確認（lint/type/test/build/security = ✅）

受入基準（Definition of Done）:
- CI green（両リポジトリ）
- 対象機能のCRUD Verify成功（証跡をPlaneコメントに添付）

### 📥 CSVインポート補助（例）

以下の列でインポート可能です（PlaneのCSVインポート機能）。
...
1. **Issue を開く**（例: REBUILD-4）
   ```
   Plane Project Board → REBUILD-4 をクリック
   ```
...
### 方法1: Plane UI で手動作成（推奨）

#### Step 1: Projectを作成

1. **Plane ホーム → 「Projects」**
2. **「New Project」クリック**
3. **入力**:
...
### 方法3: Plane API（自動化・上級者向け）

#### Step 1: Plane API Key取得

1. **Plane → Settings → API**
2. **「Create new API key」**
3. **Key をコピー（環境変数に保存）**
   ```bash
   export PLANE_API_KEY="lin_api_xxxx"
   ```
...
Step 1: Phase内の全Issue確認
  ↓
  Planeで Filter: Milestone = "Phase X"
  ↓
  全て「Done」か確認
    ↓ YES
    Step 2へ
...
Step 2: Issue を「In Progress」に戻す
  ↓
  Plane: Status を Done → In Progress
...
**PLANE_ISSUES.md の修正が完了しました（60点 → 100点）。**

### 実装AIへの指示

```
Step 1: PLANE_ISSUES.md を読む（30分）
  ↓ Plane設定方法を理解

Step 2: Plane Project・Milestones・Labelsを作成（30分）
  ↓ 環境準備完了

Step 3: Plane Issues を作成（1時間）
  ↓ REBUILD-1〜16を登録
