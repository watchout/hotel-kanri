# 📊 Plane完全依存・進捗管理セットアップ

**最終更新**: 2025年11月5日  
**方針**: PlaneをSingle Source of Truthとする
...
**最終更新**: $(date)  
**ソース**: Plane Project "hotel-saas/hotel-common リビルド"
...
git commit -m "自動: Plane進捗エクスポート $(date +%Y-%m-%d)"
git push
...
1. **タスク開始時**
   - Planeでステータスを「In Progress」に変更
   - 開始時刻を記録（Planeが自動記録）

2. **タスク完了時**
   - Planeでステータスを「Done」に変更
   - 実績工数を記録
   - 完了条件を全てチェック

3. **ブロッカー発生時**
   - Planeで「Blocker」ラベル追加
