...
## 🚀 Claude Code GitHub Actions（自動実装）

### 原則: 実装が必要なものは全て Claude Code へ

```
Plane タスク → GitHub Issue → Claude Code 自動実装 → PR作成 → レビュー
```
...
```bash
# Planeタスクから GitHub Issue を自動作成 → Claude Code が実装
node scripts/plane/dispatch-to-claude.cjs DEV-0170

# バグ修正モード
node scripts/plane/dispatch-to-claude.cjs DEV-0170 --fix

# ドライラン（確認のみ）
node scripts/plane/dispatch-to-claude.cjs DEV-0170 --dry-run
```

#### 方法2: GitHub Issue テンプレートから作成

1. https://github.com/watchout/hotel-kanri/issues/new/choose
2. 「Claude Code 実装依頼」を選択
3. テンプレートに従って入力
4. `claude-implement` ラベルが自動付与 → Claude Code が実装開始

#### 方法3: PR/Issue コメントで直接指示

```
@claude この機能を実装してください: [具体的な指示]
@claude このバグを修正してください: [バグの内容]
...
| ワークフロー | トリガー | 用途 |
|:-------------|:---------|:-----|
| `claude.yml` | @claude メンション / Issue作成 | 実装・修正・質問 |
| `claude-auto-dev.yml` | workflow_dispatch（手動） | タスク指定の自動実装 |
| `claude-code-review.yml` | PR作成/更新 | 自動コードレビュー |

### ルール

- `.claude/rules/auto-implementation.md` に従って実装される
- 禁止パターン検出時は自動停止
- SSOTに矛盾がある場合は Issue コメントで報告
- Prismaスキーマ変更が必要な場合は人間の承認を待つ
...
- [auto-implementation rule](./.claude/rules/auto-implementation.md) - 自動実装ルール
